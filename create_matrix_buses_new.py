...
def get_all_data():
	...
	filename1 = 'Stoppage_Ref_Nodes_534_up.xlsx'
	...
	for filename in glob.glob('/home/anubhav/Desktop/IP/Final/534_final/*'):
		...
		arraytimeStart = sheet.col_values(2)
		arraytimeEnd = sheet.col_values(6)
		...
		for i in range(1,(len(arrayofnode)-1)):
			...
			print arrayofnode[i]
			if(arrayofnode[i+1]!=1.0 and arraydir[i]!='down' and arraydir[i+1]!='down'):
				print i
				print filename
				print arrayofnode[i]
				...
				m2 = int(arraytimeEnd[i+1][-8])+int(arraytimeEnd[i+1][-9])*10 
				h2 = int(arraytimeEnd[i+1][-11])+ int(arraytimeEnd[i+1][-12])*10 
				s2 = int(arraytimeEnd[i+1][-5]) +int(arraytimeEnd[i+1][-6])*10
				...
				coords_1 = ((arraylat3[int(arrayofnode[i])], arraylong3[int(arrayofnode[i])]))
				coords_2 = ((arraylat3[int(arrayofnode[i+1])], arraylong3[int(arrayofnode[i+1])]))
